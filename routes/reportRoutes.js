// routes/reportRoutes.js - VERSIÓN CORREGIDA CON FILTRO MULTI-TENANT COMPLETO
...
const initReportRoutes = (models) => {
    ...
    router.get('/client-status-dashboard', 
    ...
    router.get('/client-risk/:clientId', 
    ...
    router.get('/summary', 
        ...
        async (req, res) => {
            try {
                const totalBalanceDue = await Sale.sum('balanceDue', { 
                    where: { ...req.storeFilter, isCredit: true, balanceDue: { [Op.gt]: 0 } } 
                });
            ...
            try {
                const pendingSales = await Sale.findAll({
                    ...
                    include: [
                        ...
                        { model: Payment, as: 'payments', attributes: ['id', 'amount', 'paymentDate'] }
                    ],
                    order: [['saleDate', 'DESC']]
                });
    ...
    router.get('/payments-accumulated', 
        authorizeRoles(['super_admin', 'regular_admin', 'sales_admin', 'viewer_reports']), 
        applyStoreFilter, 
        async (req, res) => {
            try {
    ...
    router.get('/collections-by-agent', 
        authorizeRoles(['super_admin', 'regular_admin', 'sales_admin', 'viewer_reports']), 
        applyStoreFilter, 
        async (req, res) => {
            try {
    ...
    // ⭐ RUTA CORREGIDA: /projected-vs-real-income
    router.get('/projected-vs-real-income', 
        authorizeRoles(['super_admin', 'regular_admin', 'sales_admin', 'viewer_reports']), 
        applyStoreFilter, 
        async (req, res) => {
            try {
                ...
                // ⭐ CORRECCIÓN CRÍTICA: Agregar filtro multi-tenant
                const activeCreditSales = await Sale.findAll({
                    where: {
                        ...req.storeFilter, // ⭐ FILTRO MULTI-TENANT AGREGADO
                        ...salesWhereClause
                    },
                    include: [{
                        model: Payment,
