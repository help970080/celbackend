...
const initReportRoutes = (models) => {
    ...
    router.get('/client-risk/:clientId', authorizeRoles(['super_admin', 'regular_admin', 'sales_admin', 'viewer_reports', 'collector_agent']), applyStoreFilter, async (req, res) => {
    ...
    router.get('/client-statement/:clientId', authorizeRoles(['super_admin', 'regular_admin', 'sales_admin', 'viewer_reports', 'collector_agent']), applyStoreFilter, async (req, res) => {
        const { clientId } = req.params;
        if (isNaN(parseInt(clientId, 10))) {
            return res.status(400).json({ message: 'El ID del cliente debe ser un número válido.' });
        }
    ...
    router.get('/sales-by-date-range', authorizeRoles(['super_admin', 'regular_admin', 'sales_admin', 'viewer_reports']), applyStoreFilter, async (req, res) => {
        try {
    ...
    router.get('/payments-by-date-range', authorizeRoles(['super_admin', 'regular_admin', 'sales_admin', 'viewer_reports']), applyStoreFilter, async (req, res) => {
        try {
            const { startDate, endDate } = req.query;
    ...
    router.get('/projected-vs-real-income', authorizeRoles(['super_admin', 'regular_admin', 'sales_admin', 'viewer_reports']), applyStoreFilter, async (req, res) => {
        try {
