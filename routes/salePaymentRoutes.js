...
const initSalePaymentRoutes = (models, sequelize) => {
    ...
    router.get('/', authorizeRoles(['super_admin', 'regular_admin', 'sales_admin']), applyStoreFilter, async (req, res) => {
        try {
            ...
            let whereClause = { ...req.storeFilter }; // ⭐ NUEVO
    ...
    router.get('/export-excel', authMiddleware, authorizeRoles(['super_admin', 'regular_admin', 'sales_admin', 'viewer_reports']), async (req, res) => {
        try {
            ...
            sales.forEach(sale => {
                ...
                worksheet.addRow({
                    id: sale.id,
                    saleDate: moment(sale.saleDate).toDate(),
