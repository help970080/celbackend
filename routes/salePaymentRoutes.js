// Archivo: routes/salePaymentRoutes.js (Versión Optimizada)
...
const initSalePaymentRoutes = (models, sequelize) => {
    ...
    // --- INICIO DE LA MODIFICACIÓN: RUTA GET / OPTIMIZADA ---
        ...
        try {
            ...
            if (search) {
                baseOptions.include = [{
                    ...
                    attributes: [] // No necesitamos traer los atributos del cliente en este paso
                }];
            }

            // Paso 1: Obtener el conteo total y los IDs de las ventas de la página actual
            const { count, rows: salesWithIds } = await Sale.findAndCountAll(baseOptions);
            const saleIds = salesWithIds.map(sale => sale.id);

            // Paso 2: Obtener todos los datos detallados solo para los IDs de la página actual
            const sales = await Sale.findAll({
                ...
                order: [['saleDate', 'DESC']], // Mantener el mismo orden
            });
        ...
        try {
            ...
            try {
                await AuditLog.create({
                    ...
                    details: `Venta ID: ${req.params.saleId}. Cambio de: ${previousCollector} a: ${newCollector}`
                });
            } catch (auditError) { console.error("Error al registrar en auditoría:", auditError); }
